...
RSpec.describe "Images", type: :request do
  ...
  describe "filter on location and ids" do
    include_context "db_clean_all"
    before(:each) do
      images = (1..5).map {|idx| FactoryGirl.create(:image,image_content:nil)}
      @ids = [images[0].id, images[3].id, images[4].id]
      @origin=FactoryGirl.build(:point)
      DistanceCollection.new(images).set_distance_from(@origin)
      @distance=images[0].distance
    end

    it "list all images within distance from origin" do
      results=Image.within(@distance, :origin=>@origin)
      jget images_path, {miles:@distance}.merge(@origin.to_hash)
      expect(response).to have_http_status(:ok)
      payload=parsed_body
      expect(payload.size).to eq(results.size)
    end

    it "include specific images in list" do
      jget images_path, {include:@ids}
      expect(response).to have_http_status(:ok)
      payload=parsed_body
      expect(payload.size).to eq(@ids.size)
    end

    it "exclude specific images from list" do
      jget images_path, {exclude:@ids}
      expect(response).to have_http_status(:ok)
      payload=parsed_body
      expect(payload.size).to eq(5-@ids.size)
    end
  end
end
